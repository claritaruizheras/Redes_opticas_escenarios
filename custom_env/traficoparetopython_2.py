...
random_loads =  [random.choice([i / 10 for i in range(1, 11)]) for _ in range(num_ont)]
...
def simular_trafico(onts=None):
    ...
    if onts is None:
        onts = [ONT(id=i, bit_rate=ONU_bit_rate, packet_size=packet_size, load=random_loads[i], streams=streams) for i in range(num_ont)] 
